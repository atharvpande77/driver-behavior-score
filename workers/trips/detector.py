...
from workers.trips.utils import new_open_trip, segment_distance

GAP_THRESHOLD_SECONDS: float = 1800.0        # 30 min — safety net for silent/dead devices

NIGHT_START_HOUR: int = 20   # 20:00 inclusive
NIGHT_END_HOUR: int = 5      # 05:00 exclusive  → night = [20:00, 05:00)
...
MIN_SPEED_THRESHOLD_KMPH: float = 2.0  # speeds below this are noise/idle and excluded from min
...
def _accumulate_speed(trip: OpenTrip, speed: float | None) -> None:
...
def _accumulate_harsh_events(trip: OpenTrip, packet_type: str | None) -> None:
    if packet_type == "HA":
...
def _close_trip(trip: OpenTrip, end_event_id: int, ended_at, end_lat, end_lon, distance_km: float) -> CloseTripAction:
        ...
        if open_trip is not None:
            ...
            if gap > GAP_THRESHOLD_SECONDS:
                ...
                if ignition is True:
                    ...
                    _accumulate_speed(open_trip, event.speed)
                    _accumulate_harsh_events(open_trip, event.packet_type)
                ...
                d_day, d_night, s_day, s_night = _day_night_split(
