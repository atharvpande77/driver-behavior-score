...
Usage (from the project root):
    python -m telematics.backfill_parsed_fields
...
single transaction per batch. Each row is wrapped in its own savepoint so
a failure on one row rolls back only that row without aborting the batch.
"""
